...
===== _Recruiting event announced (event → actions → follow-up events)_
...
===== _Candidate checked in (event interleaved with action)_
* **Type:** event  
...
===== _Queue formed in front of booth (event explained in flow)_
* **Type:** event  
...
===== _Recruiter solicited introduction (action → event sequence)_
...
===== _Resume/portfolio handed over (action → event + entity appearance)_
* **Type:** hybrid  
...
===== _Recruiter recording notes (operation → event clearly separated per feedback)_
...
===== _Informal meetup occurred (event with embedded actions)_
* **Type:** event  
...
===== _Multichannel application submitted (action → event → entity)_
* **Type:** hybrid  
...
===== _Pre-screen completed (operation → event)_
* **Type:** event  
...
===== _Invitation/follow-up delayed (event contextualized)_
* **Type:** event  
...
===== _Decision to stop sourcing (operation → event)_
* **Type:** operation + event  
...
| DR004 | The system must record the number of available positions for each opening. | US-Recruiter-001: Post New Opening | T-DB-003

| DR005 | The system must link every opening to its responsible employer entity. | US-Recruiter-001: Post New Opening | T-API-003

| DR006 | The system must allow requirement updates for an opening. | US-Recruiter-002: Update Opening | T-Audit-001

| DR007 | The system must preserve a complete history of all requirement changes for an opening (audit trail). | US-Recruiter-002: Update Opening | T-Audit-001

| DR008 | The system must represent each candidate profile with education, work history, competencies, certifications, portfolios, and verifiable references. | Epic-ProfileMgmt | T-DB-004

| DR009 | The system must allow documents to be attached to the candidate profile. | US-Candidate-001: Upload Resume | T-File-001

| DR010 | The system must preserve the issue date, validity, and verification status for all attached documents. | US-Candidate-001: Upload Resume | T-File-001

| DR0011 | The system must record candidate preferences such as areas of interest, desired location, and work modality. | US-Candidate-002: Set Preferences | T-DB-005

| DR012 | The system must represent recruiting events with name, organizer, venue, agenda, date, and capacity. | Epic-EventMgmt | T-DB-006, T-API-004

| DR013 | The system must represent company booths/tables within an event structure. | Epic-EventMgmt | T-DB-006, T-API-004

| DR014 | The system must register the attendance and arrival of candidates and recruiters at an event. | US-Event-001: Check-in at Event | T-Event-001

| DR015 | The system must enforce event registration rules and capacity limits upon check-in. | US-Event-001: Check-in at Event | T-API-005

| DR016 | The system must model queues in front of event booths/tables. | Epic-QueueMgmt | T-Queue-001

| DR017 | The system must enforce a first-come, first-served order for modeled queues. | Epic-QueueMgmt | T-Queue-001

| DR018 | The system must allow limiting the duration of each queue turn. | US-Event-002: Manage Queue | T-Queue-002

| DR019 | The system must register the closure of each turn. | US-Event-002: Manage Queue | T-Queue-002

| DR020 | The system must prevent assigning turns that would exceed declared capacity or time limits. | US-Event-002: Manage Queue | T-Queue-002

| DR021 | The system must allow candidates and recruiters to express positive or negative interest in openings or profiles. | US-Interaction-001: Express Interest | T-API-006

| DR022 | The system must record reciprocal expressions of interest. | US-Interaction-001: Express Interest | T-API-006

| DR023 | The system must create a Match record when both sides express positive interest. | US-Interaction-002: Create Match | T-Match-001

| DR024 | The system must preserve the date and context of every Match creation event. | US-Interaction-002: Create Match | T-Match-001

| DR025 | The system must allow recruiters to define shortlisting criteria and register the decision. | US-Recruiter-003: Shortlist Candidate | T-API-007

| DR026 | The system must record the justification for every shortlisting or rejection decision. | US-Recruiter-003: Shortlist Candidate | T-API-007

| DR027 | The system must allow candidates to withdraw their interest from an opening or Match. | US-Candidate-003: Withdraw Interest | T-Match-002

| DR028 | The system must automatically update any pending Matches when interest is withdrawn. | US-Candidate-003: Withdraw Interest | T-Match-002
...
| IR001 | The system must provide a structured registration interface for new recruiters, including fields for company name, professional email, and job role. | US-Auth-002: Register as Recruiter | T-UI-010, T-FE-001
...
| IR004 | The internal skill representation must be updated within a maximum of 300 ms after the user saves changes, ensuring a measurable, achievable update time instead of an absolute immediate requirement. | | T-BE-002, T-Perf-001
...
| IR010 | The system must update the internal state within 200–350 ms to reflect a user's swipe action on a profile card (recording the decision and removing the card), ensuring the round-trip time for card replacement meets performance expectations. | US-Interaction-001: Express Interest | T-FE-003, T-Perf-002
...
| MR001 | Developer workstations must meet a minimum specification of Dual-core processor (Intel i5/AMD equivalent), 8 GB RAM, 256 GB SSD, and GPU capable of supporting Flutter/Android/iOS emulators efficiently. | Epic-DevSetup | T-Env-001

| MR002 | Mobile devices for testing must include Android devices running Android 10.0+ with at least 3 GB RAM and iOS devices running iOS 15+ (e.g., iPhone 11 or newer). Devices must support automated UI testing frameworks where applicable. | Epic-Testing | T-Env-002
...
| MR004 | The Flutter SDK (latest stable release) and Dart SDK (bundled with Flutter) must be used for development. Development environment must support hot reload and debugging tools. | | T-Tool-001

| MR005 | Android Studio and Xcode must be used for builds and emulators/simulators, including latest stable versions and plugins for Flutter development. | | T-Tool-002

| MR006 | Git and GitHub must be used for version control and collaboration; Git workflow must include feature branches and pull requests. | | T-Tool-003
...
| MR009 | Developer machines must run Windows 10/11 or macOS Monterey+ with all necessary dependencies installed. | | T-Env-004
  
| MR010 | The application must be deployed to targets running Android 10+ and iOS 15+. | | T-Env-005

| MR011 | A stable broadband connection (≥10 Mbps) must be available for syncing and testing cloud services; latency should not exceed 100ms in normal conditions. |  | T-Env-006
...
Each section of the documentation (terminology, requirements, narrative) will be reviewed by a different team member. Reviews confirm consistent terminology, measurable requirements, and proper linkage to design elements.  
  Early reviews revealed ambiguities between the terms “candidate” and “student,” which were clarified.

* **Checklists**  
  A milestone-based checklist will be used to explicitly verify each requirement and trace it to design and implementation elements:  
...
* **Traceability Matrix (Planned)**  
  A lightweight traceability matrix will be maintained to link requirements to goals, terminology, verification methods, and design elements. This ensures all verification is explicit and measurable:  
...
* **Walkthroughs of Documentation**  
  Reader walkthroughs will be simulated by having a team member act as an external reviewer to check that each requirement traces back to definitions, models, and implementation elements. This identifies undefined or ambiguous terms and confirms measurable verification coverage.  
