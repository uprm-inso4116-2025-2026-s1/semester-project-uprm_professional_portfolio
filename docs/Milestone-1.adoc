...
To maintain clarity and prevent ambiguity, the domain logic is captured through explicit function signatures. These serve as contracts between layers of the architecture.
...
Each function encodes a clear business rule. For example, `processSwipe` ensures that only valid swipe actions are stored, while `checkForMatch` guards the creation of matches until both parties express interest. This separation of concerns keeps the domain logic auditable and transparent.

===== Example: Swiping Interaction Flow

When a candidate views a recruiter’s profile card, the sequence of calls is as follows:

. *Deck generation* → `getUserDeck : UserId -> Deck` builds the candidate’s deck based on preferences and availability.  
. *Profile presentation* → `getNextProfile` surfaces the next card.  
. *Gesture capture* → A Like/Pass is processed through `processSwipe`.  
. *Match evaluation* → The system triggers `checkForMatch`, and if successful, a `createConnection` is invoked.  
. *Notification* → Both parties receive an in-app notification of the new connection.  

This fragment demonstrates how the conceptual “Tinder-like” behavior translates into a precise flow with defined entry and exit points.

===== Example: Event Participation

Event logic complements the swiping flow by offering an additional interaction channel:

[source,scala]
----
// Candidate RSVPs for an event
rsvpEvent : UserId >< EventId -> Result<Attendance, Error>

// Notify candidate 24h before the event
sendReminder : EventId >< UserId -> Notification
----

This ensures that events are integrated seamlessly with candidate–recruiter interactions, enforcing constraints like capacity and timely communication.

===== User Interface Complement

Below is a conceptual sketch of the *Profile Card* UI component. It reflects the condensed information hierarchy surfaced during swiping:

* *Header:* Candidate name / Recruiter organization.  
* *Body:* Key skills, eligibility, and location.  
* *Footer:* Action buttons (Like, Pass, More Info).  

This design complements the textual logic by showing how the domain concept of a *ProfileCard* manifests visually in the system.

===== Rationale for Including These Fragments

Natural language descriptions of swiping, matching, and event participation quickly become cumbersome. By including the code-like signatures, the intended behavior is made precise while remaining platform-independent. The accompanying UI sketch ties the abstract logic back to user experience, showing how requirements map into implementation.

==== Architecture vs. Design

At the *software architecture* level, the system is divided into three main components:

. *Mobile Frontend* – provides the user experience for candidates and recruiters through onboarding, profile cards, swiping, and messaging.  
. *Application Backend* – enforces business rules (swiping, matching, event capacity) and orchestrates communication with external services.  
. *Data Layer* – persists entities such as profiles, swipes, matches, events, and messages.  

These components are related in a layered structure: the frontend issues commands, the backend enforces domain rules, and the data layer ensures consistency and durability.

At the *software design* level, each architectural component is realized through specific responsibilities. For example, the *Matching Service* in the backend design is responsible for evaluating swipes, confirming matches, and triggering notifications. Its logic is expressed through explicit operations.

===== Example: Swiping and Matching Flow

When a candidate swipes right on a recruiter’s profile card, the system records the action and checks for a reciprocal swipe. If both parties have liked each other, the *Matching Service* establishes a `Connection` object and immediately notifies both users.

[source,scala]
----
// Domain service for processing swipes
class MatchingService {
    def processSwipe(userId: UserId, profileId: ProfileId, direction: SwipeDirection): Result[Unit, Error] = {
        // Persist swipe in repository
        swipeRepository.save(userId, profileId, direction)

        // If Like, check for mutual interest
        if (direction == SwipeDirection.Like) {
            checkForMatch(userId, profileId) match {
                case Some(matchObj) => createConnection(matchObj)
                case None => ()
            }
        }
    }
...
This fragment demonstrates the *Single Responsibility Principle (SRP)*: the service handles swiping logic only, delegating persistence to the repository and connection creation to another component.

[plantuml, swiping-flow, svg]
----
@startuml
actor User
rectangle Frontend
rectangle "Matching Service" {
    Frontend --> "Matching Service" : Swipe Action
    "Matching Service" --> "Swipe Repository" : Save Swipe
    "Matching Service" --> "Connection Service" : Check & Create Match
    "Connection Service" --> "Notification Service" : Send Notification
}
@enduml
----

The diagram complements the code fragment by showing how swiping logic sits within the overall architecture.

===== Example: Event RSVP Flow

Recruiting events are another key interaction point. Candidates can RSVP, and the system enforces event capacity while also sending reminders 24 hours before the event.

[source,scala]
----
def rsvpEvent(userId: UserId, eventId: EventId): Result[Attendance, Error] = {
    if (eventRepository.hasCapacity(eventId)) {
        val attendance = Attendance(userId, eventId, confirmed = true)
        eventRepository.saveAttendance(attendance)
        return Result.success(attendance)
    }
    Result.failure(CapacityReachedError)
}
----

This example illustrates the *Open/Closed Principle (OCP)*: the RSVP logic is extensible (for example, to support waitlists later) without changing the core.
...
rectangle "Event Service" {
    Candidate --> Frontend : RSVP
    Frontend --> "Event Service" : RSVP Request
    "Event Service" --> "Event Repository" : Save Attendance
    "Event Service" --> "Notification Scheduler" : Schedule Reminder
}
@enduml
----
